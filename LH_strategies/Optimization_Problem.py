...
class EVM():
    ...
    def OptimizeModel(self):
        ...
        self.model.optimize()
        if self.model.IsMIP:
            opt_gap = self.model.MIPGap if self.model.SolCount > 0 else None
        else:
            opt_gap = 0.0  # continuous model → no gap
        time_run = self.model.Runtime
        obj_value = self.model.ObjVal
        return opt_gap, time_run, obj_value
