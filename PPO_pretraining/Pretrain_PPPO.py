...
sys.path.append("..\\env_1timescale")   
sys.path.append("..\\pretraining_1timescale")   
...
eval_callback = EvalCallback(env, eval_freq=eval_timesteps, 
                             log_path="./PPPO_evaluations_1e-5/instance_"+str(parameter)+"/", 
                             n_eval_episodes = eval_episodes, deterministic=True, verbose = 0.0)

###### Pretraining ############

expert_obs = np.load("./expert_datasets/expert_observations_"+str(parameter)+".npy")
expert_actions = np.load("./expert_datasets/expert_actions_"+str(parameter)+".npy")
...
from Pretraining import pretrain_agent
...
from Pretraining import pretrain_vf
...
#mean_reward, _ = evaluate_policy(pppo, env, n_eval_episodes=300, return_episode_rewards = True)

#with open("PPPO_evaluations_1e-5/instance_"+str(parameter)+"/average_reward.txt", "w") as f:
#    f.write(str(np.mean(mean_reward)))
    
#print('Task number: ', parameter, f"Mean reward = {np.mean(mean_reward)}")

#np.save("./PPPO_evaluations_1e-5/instance_"+str(parameter)+"/evaluations.npy", mean_reward)

## Active Training ##

pppo.learn(total_timesteps = n_timesteps, callback = eval_callback)
print('Task number: ', parameter, f"Time of active learning: {(time.time()-Start_time)/60} minutes")


End_time = (time.time()-Start_time)/60
with open("PPPO_evaluations_3e-6/instance_"+str(parameter)+"/runtime.txt", "w") as f:
...
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/x_PPPO.npy", x_m)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/y_PPPO.npy", y_m)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/p_PPPO.npy", p_m)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/d_PPPO.npy", d_m)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/c_PPPO.npy", c_m)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/lambda_PPPO.npy", lambda_m)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/f_PPPO.npy", f_m)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/w_PPPO.npy", w_m)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/r_PPPO.npy", r_m)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/z_PPPO.npy", z_m)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/Revenue_PPPO.npy", Revenue_v)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/LostSales_PPPO.npy", LostSales_v)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/Repositioning_PPPO.npy", Repositioning_v)
#np.save("./PPPO_simulations_1e-5/instance_"+str(parameter)+"/Profit_PPPO.npy", Profit_v)
