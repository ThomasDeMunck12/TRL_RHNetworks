...
variance_trip_durations = np.zeros([N, N])

for i in range(N):
    for j in range(N):
        x = df_8loc[((df_8loc['PULocationID'] == i) & (df_8loc['DOLocationID']==j))] 
        y = (x["trip_time"] / 300).mean()
        z = (x["trip_time"] / 300).std()
        average_trip_durations[i,j] = y
        variance_trip_durations[i,j] = z
        
np.save("average_trip_durations_8loc", average_trip_durations)

(df_8loc["trip_time"] / 300).hist(density=True, bins=500)
plt.xlabel("Values")
...
#b.  large instances - to be continued on Julia due to the amount of required computational effort

df_20loc.to_csv('df_20.csv')
df_20loc_march.to_csv('df_20_march.csv')

# Compute mean and std
mu = (df_8loc["trip_time"] / 300).mean()
sigma = (df_8loc["trip_time"] / 300).std()

# Generate normal PDF
x_vals = np.linspace(0, (df_8loc["trip_time"] / 300).max(), 500)
y_vals = (1 / (sigma * np.sqrt(2*np.pi))) * np.exp(-0.5 * ((x_vals - mu)/sigma)**2)

plt.plot(x_vals, y_vals, linewidth=2, label="Normal")

# Generate exponential PDF

lam = 1 / mu
y_exp = lam * np.exp(-lam * x_vals)
plt.plot(x_vals, y_exp, linestyle="--", label="Exponential")

plt.plot(x_vals, y_vals, linewidth=2, color='red')

# Generate uniform PDF

a = (df_8loc["trip_time"] / 300).min()
b = (df_8loc["trip_time"] / 300).max()
y_unif = np.where((x_vals >= a) & (x_vals <= b), 1/(b - a), 0)
plt.plot(x_vals, y_unif, linestyle=":", label="Uniform")
